...
int main()
{
  try
  {
    ...
    ///////////////////////// TO-DO (1) //////////////////////////////
    // Creating a base grocery list for the week
GroceryList weeklyGroceryList;
weeklyGroceryList += {{"Marshmallows"}, {"Cranberry Juice"}, {"Rosemary"}, {"Shaving Cream"}, {"Mango Juice"},
                      {"Ice"}, {"Frozen Peas"}, {"Pita"}, {"Beer"}, {"Ginger"},
                      {"Cilantro"}, {"Cottage Cheese"}, {"Cream Cheese"}, {"Butternut Squash"}, {"Basil"}};

// Realized we need to prioritize fresh herbs for early-week cooking
weeklyGroceryList.moveToTop({"Cilantro"});
weeklyGroceryList.moveToTop({"Basil"});
weeklyGroceryList.moveToTop({"Rosemary"});

// Decided to make a special cocktail, need to move "Cranberry Juice" up
weeklyGroceryList.insert({"Cranberry Juice"}, 1); // Re-insert at a higher priority

// Combining with a roommate's list, who also needs some items
GroceryList roommatesList = {{"Toothpaste"}, {"Paper Towels"}, {"Cranberry Juice"}, {"Almond Milk"}};
if (weeklyGroceryList != roommatesList) {
    weeklyGroceryList += roommatesList;
}

// Removing duplicates and unwanted items
weeklyGroceryList.remove({"Beer"});
weeklyGroceryList.remove({"Shaving Cream"});

// Final review of the list before going to the store
std::cout << "Final Weekly Grocery List:" << std::endl;
std::cout << weeklyGroceryList << std::endl;

// Validate the final list with what's expected
GroceryList expectedList = {{"Rosemary"}, {"Basil"}, {"Cilantro"}, {"Marshmallows"}, {"Cranberry Juice"},
                            {"Mango Juice"}, {"Ice"}, {"Frozen Peas"}, {"Pita"}, {"Ginger"},
                            {"Cottage Cheese"}, {"Cream Cheese"}, {"Butternut Squash"}, {"Toothpaste"},
                            {"Paper Towels"}, {"Almond Milk"}};
std::cout << "\nComparison with Expected List: " << (weeklyGroceryList == expectedList ? "All Match" : "Does Not Match") << std::endl;
