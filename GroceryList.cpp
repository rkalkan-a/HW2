...
if (it != _gList_vector.end()) {
  ...
  // Item found - calculate and return the zero based indexed
...
if (find(groceryItem) != size()) {
  // Item already exists, so we silently discard it
  return;
}
...
auto numPositions = _gList_array_size - offsetFromTop;
if (numPositions > 0) {
    std::shift_right(_gList_array.begin() + offsetFromTop, _gList_array.begin() + _gList_array_size, numPositions);
}
...
++_gList_array_size;
  ...
  { /**********  Part 2 - Insert into vector  **********************/
    ///////////////////////// TO-DO (5) //////////////////////////////
      ...
      auto insertPos = std::next(_gList_vector.begin(), offsetFromTop);
      _gList_vector.insert(insertPos, groceryItem);
...
auto insertPosDLL = std::next(_gList_dll.begin(), offsetFromTop);
// Use list's insert function to insert the item before the calculated
_gList_dll.insert(insertPosDLL, groceryItem);
    /////////////////////// END-TO-DO (6) ////////////////////////////
  } // Part 3 - Insert into doubly linked list
...
if (offsetFromTop == 0) {
    ...
    _gList_sll.insert_after(_gList_sll.before_begin(), groceryItem);
} else {
    // For insertion at positions other than the beginning, find the correct position
    auto insertPosSLL = _gList_sll.before_begin();
    // Advance the iterator offsetFromTop - 1 times to get the position just before where you want to insert
    for (std::size_t i = 0; i < offsetFromTop; ++i) {
        ++insertPosSLL;
    }
    // Now insert after the found position
    _gList_sll.insert_after(insertPosSLL, groceryItem);
}
    /////////////////////// END-TO-DO (7) ////////////////////////////
  } // Part 4 - Insert into singly linked list
...
// Correct usage of std::shift_left
if (offsetFromTop < _gList_array_size) {
    auto numPositions = _gList_array_size - offsetFromTop - 1;
    if (numPositions > 0) {
        std::shift_left(_gList_array.begin() + offsetFromTop, _gList_array.begin() + _gList_array_size, numPositions);
    }
    --_gList_array_size;
}
    /////////////////////// END-TO-DO (8) ////////////////////////////
  ...
  {/**********  Part 4 - Remove from singly linked list  **********/
    ...
    // For removing at positions other than the beginning
    auto it = _gList_sll.before_begin();
    // Advance the iterator offsetFromTop - 1 times to reach the element just before the one to remove
    for (std::size_t i = 0; i < offsetFromTop; ++i) {
        ++it;
    }
    // Now erase after the found position
...
std::weak_ordering GroceryList::operator<=>( GroceryList const & rhs ) const
{
  ...
  size_t minLength = std::min(_gList_vector.size(), rhs._gList_vector.size());

for (size_t i = 0; i < minLength; ++i) {
    auto cmp = _gList_vector[i] <=> rhs._gList_vector[i];
    if (cmp != 0) { // If any difference is found, return it immediately
      return cmp;
    }
  }

  // If reached here, all items in the common extent are equal; compare the sizes
...
bool GroceryList::operator==( GroceryList const & rhs ) const
{
  ...
  if (_gList_vector.size() != rhs._gList_vector.size()) return false;
    // Check if any element is different
    return std::equal(_gList_vector.begin(), _gList_vector.end(), rhs._gList_vector.begin());
  /////////////////////// END-TO-DO (16) ////////////////////////////
}
...
std::istream & operator>>( std::istream & stream, GroceryList & groceryList )
{
  ...
  if (!groceryList.containersAreConsistant()) {
    throw GroceryList::InvalidInternalState_Ex(make_details("Container consistency error"));
  }
  
  GroceryItem item;
  while (stream >> item) { // Assuming the overloaded operator>> for GroceryItem handles the input correctly
